...
class DeviceDirectory:
    ...
    def size_check(self) -> int:
        total_size: int = sum(file.size for file in self._files)
        for d in self._directories:
            total_size += d.size_check()
        return total_size
...
if __name__ == '__main__':
    with open('../input.txt', 'r') as input_file:
        ...
        total_used = root_directory.size
        total_needed = 30000000 - (70000000 - total_used)
        all_dirs.sort(key=lambda d: d.size)
        for i_dir in all_dirs:
            if i_dir.size > total_needed:
                print(i_dir.name)
                print(i_dir.size)
                break